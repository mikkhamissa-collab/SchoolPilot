...
class GradeEntry(BaseModel):
    ...
    max: float = Field(default=100, gt=0, le=10000)


class Policies(BaseModel):
    drop_lowest: dict[str, int] = {}
    missing_penalty: float = Field(default=0, ge=0, le=100)
...
class WhatIfRequest(BaseModel):
    categories: list[GradeCategory]
    grades: list[GradeEntry]
    hypothetical_category: str = Field(min_length=1, max_length=200)
    hypothetical_score: float = Field(ge=0, le=10000)
    hypothetical_max: float = Field(default=100, gt=0, le=10000)
    policies: Optional[Policies] = None
...
class RequiredScoreRequest(BaseModel):
    ...
    target_category: str = Field(min_length=1, max_length=200)
    policies: Optional[Policies] = None
