...
// Handles user messages (right-aligned), assistant messages (left-aligned) with
// markdown-like formatting, and inline action cards for tool results.
...
function renderMarkdown(text: string): React.ReactNode[] {
  ...
  for (const line of lines) {
    ...
    if (trimmed.startsWith("### ")) {
      flushList();
      elements.push(
        <h4 key={`h4-${key++}`} className="text-sm font-bold text-white mt-2 mb-0.5">
    ...
    if (trimmed.startsWith("## ")) {
      flushList();
      elements.push(
        <h3 key={`h3-${key++}`} className="text-sm font-bold text-white mt-2 mb-0.5">
...
function ReminderCard({ action }: { action: ActionTaken }) {
  ...
  return (
    <div className="flex items-start gap-2.5 p-2.5 rounded-lg bg-accent/10 border border-accent/20 mt-1.5">
...
function ProfileUpdateCard({ action }: { action: ActionTaken }) {
  ...
  return (
    <div className="flex items-start gap-2.5 p-2.5 rounded-lg bg-success/10 border border-success/20 mt-1.5">
...
function GradeAnalysisCard({ action }: { action: ActionTaken }) {
  ...
  return (
    <div className="flex items-start gap-2.5 p-2.5 rounded-lg bg-warning/10 border border-warning/20 mt-1.5">
      <div className="w-7 h-7 rounded-full bg-warning/20 flex items-center justify-center shrink-0 mt-0.5">
        <ChartIcon />
      </div>
      <div className="min-w-0">
        <p className="text-xs font-medium text-white">{course}</p>
...
function GenericActionCard({ action }: { action: ActionTaken }) {
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0">
        <p className="text-xs font-medium text-white">{action.tool.replace(/_/g, " ")}</p>
        {hasError && (
    ...
    <div className={`flex ${isUser ? "justify-end" : "justify-start"} mb-3`}>
      ...
      >
        ...
        {isUser ? (
          ...
          <div className="space-y-0">{formattedContent}</div>
        )}

        {/* Streaming cursor */}
        {isStreaming && !isUser && (
          <span className="inline-block w-1.5 h-4 bg-accent/70 rounded-sm ml-0.5 animate-pulse align-middle" />
        )}
